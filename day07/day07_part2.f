     ...
     &      counts(card_value(hand(i:i))) + 1
      ...
      function hand_value_part2(hand) result(max_value)
        implicit none
        character*5, intent(in) :: hand
        character*5 :: hand_temp, max_hand
        character, dimension(13), parameter :: cards = [
     &   'J', '2', '3', '4', '5', '6', '7', '8', '9', 'T', 'Q', 'K', 'A'
     &  ]
        integer :: value, i, j, max_value
        ...
        do i=1,13
          do j=1,LEN(hand)
            if (hand(j:j) == 'J') then
              hand_temp(j:j) = cards(i)
            else
              hand_temp(j:j) = hand(j:j)
            end if
          end do

          value = hand_value(hand_temp)
  ...
  200 end subroutine bubblesort_hand
      ...
      subroutine bubblesort_hands(hands, bids)
        ...
        do
          ...
          do i=2,n
            d = hand_value_part2(hands(i - 1)) 
     &            - hand_value_part2(hands(i))
            if (d == 0) then
